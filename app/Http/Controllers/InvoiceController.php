...
class InvoiceController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        if (request('invoice') != '') {
            ...
            $this->type = 'invoice';
            $this->invoice = Invoice::with('items', 'unit', 'client', 'client.projects')->findOrFail($this->invoiceId);
        }
    ...
    {
        $this->invoiceSetting = invoice_setting();
        $this->invoice = Invoice::with('project', 'unit')->findOrFail($id)->withCustomFields();
    ...
    {
        $this->invoiceSetting = invoice_setting();
        $this->invoice = Invoice::with('items', 'unit')->findOrFail($id)->withCustomFields();
        App::setLocale($this->invoiceSetting->locale);
        Carbon::setLocale($this->invoiceSetting->locale);
    ...
    public function update(UpdateInvoice $request, $id)
    {
        ...
        return Reply::redirect(route('invoices.index'), __('messages.updateSuccess'));
    }

    public function show($id)
    {
        $this->invoice = Invoice::with('project', 'items', 'unit', 'items.invoiceItemImage')->findOrFail($id)->withCustomFields();
    ...
    {
        $invoice = Invoice::with(['project', 'project.client', 'unit'])->findOrFail($invoiceID);
    ...
    {
        $categorisedProduct = Product::where('category_id', $request->id)->get();
